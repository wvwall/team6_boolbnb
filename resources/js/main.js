...
let app1 = new Vue({
    ...
    mounted() {
        ...
        if (this.search_generic != null) {
            ...
            axios
                .get(
                    `https://api.tomtom.com/search/2/search/${this.search_generic}.json?key=${this.key}`
                )
                .then(response => {
                ...
                .then(response => {
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        ...
                        if (distancekm <= range) {
                            ...
                            console.log(
                                "trovato!" + response.data[i] + distancekm
                            );
                        }
    ...
    methods: {
        ...
        addressSugg: function(address, city) {
            ...
            axios
                .get(
                    `https://api.tomtom.com/search/2/search/${this.userAddress}%20${this.userCity}.json?key=${this.key}`
                )
                .then(response => {
        ...
        searchApartment: function(input) {
            if (input != "") {
                ...
                axios
                    .get(
                        `http://localhost:8000/api/apartments/search/${this.inputQueryFuz}`
                    )
                    .then(response => {
                        console.log(response.data);
                        this.queryApartmentResult = response.data;
        ...
        cambiaPosizione: function(city) {
            ...
            axios
                .get(
                    `https://api.tomtom.com/search/2/search/${city}.json?key=${this.key}`
                )
                .then(response => {
                    this.results = response.data.results;
                    console.log(this.results[0]);
